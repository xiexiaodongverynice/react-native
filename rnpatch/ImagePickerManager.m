        ...
        if ([mediaType isEqualToString:(NSString *)kUTTypeImage]) { // PHOTOS
            ...
            if (imageURL && [[imageURL absoluteString] rangeOfString:@"ext=GIF"].location != NSNotFound) {
                ...
                [assetsLibrary assetForURL:imageURL resultBlock:^(ALAsset *asset) {
                    ...
                    [gifResponse setValue:@(originalImage.size.width) forKey:@"width"];
                    [gifResponse setValue:@(originalImage.size.height) forKey:@"height"];

                    BOOL vertical = (originalImage.size.width < originalImage.size.height) ? YES : NO;
                    [gifResponse setValue:@(vertical) forKey:@"isVertical"];

                    if (![[self.options objectForKey:@"noData"] boolValue]) {
                        NSString *dataString = [data base64EncodedStringWithOptions:0];
                        [gifResponse setValue:dataString forKey:@"data"];
                    }

                    NSURL *fileURL = [NSURL fileURLWithPath:path];
                    [gifResponse setValue:[fileURL absoluteString] forKey:@"uri"];

                    NSNumber *fileSizeValue = nil;
                    NSError *fileSizeError = nil;
                    [fileURL getResourceValue:&fileSizeValue forKey:NSURLFileSizeKey error:&fileSizeError];
                    if (fileSizeValue){
                        [gifResponse setValue:fileSizeValue forKey:@"fileSize"];
                    }
            ...
            [fileURL getResourceValue:&fileSizeValue forKey:NSURLFileSizeKey error:&fileSizeError];
            if (fileSizeValue){
                [self.response setValue:fileSizeValue forKey:@"fileSize"];
            }

            [self.response setValue:@(editedImage.size.width) forKey:@"width"];
            [self.response setValue:@(editedImage.size.height) forKey:@"height"];
            ...
            [self.response setValue:videoDestinationURL.absoluteString forKey:@"uri"];
            if (videoRefURL.absoluteString) {
                [self.response setValue:videoRefURL.absoluteString forKey:@"origURL"];
            }
