...
class DatePicker extends Component {
  constructor(props) {
    ...
    this.renderAllowClear = this.renderAllowClear.bind(this);
  }
  ...
  getTitleElement() {
    const { date, placeholder, customStyles, validateFail, disabled } = this.props;
    if (validateFail) {
      return (
        <Text style={[Style.placeholderText, customStyles.placeholderText, { color: 'red' }]}>
          {placeholder}
        </Text>
      );
    ...
    return (
      <Text style={[Style.dateText, disabled && customStyles.dateText]}>{this.getDateStr()}</Text>
    );
  }
  ...
  //清空时间
  allowClear() {
    const upProps = _.get(this.props, 'upProps');
    const onChange = _.get(upProps, 'onChange');
    const handleValueChange = _.get(upProps, 'handleValueChange');
    const fields = _.get(upProps, 'field');
    const fieldName = _.get(fields, 'field');
    onChange && onChange(null);
    handleValueChange(fieldName, null);
  }

  renderAllowClear() {
    const { date, disabled, validateFail, placeholder } = this.props;
    let is_allowClear = false;
    if (disabled) {
      is_allowClear = false;
      return;
    } else {
  ...
  render() {
    ...
    const is_allowClear = !disabled && date;

    return (
      <View style={{ flex: 1, flexDirection: 'row', ...wrapSyle }}>
        <TouchableComponent
        ...
        {!hideClear && this.renderAllowClear()}
      </View>
    );
  }
}
